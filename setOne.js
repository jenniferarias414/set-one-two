...
// CODE HERE
const printParameters = (para1, para2) => {return `The first parameter is ${para1}. The second parameter is ${para2}`}
//return with {}, otherwise neither

// INVOKE THE FUNCTION HERE. THE PARAMETERS TAKE ANY DATATYPE.
console.log(printParameters('Bob', 'Sue'))
...
lastName of your choice, and a callback function that takes a parameter of fullName. The callback function 
...
// console.log(greeting('Jenny', 'Arias', callback))
greeting("Jenny", "Arias", (fullName) =>
...
Inside the function, loop over the 'prices' array. For every iteration, calculate a new total cost 
(original price plus 20% tax), then push it to the 'totalCost' array. After the loop, invoke the callback function, 
...
//         let price[i] = price[i] + (price[i] * .2)
            // totalCost.push(price[i])
//     }
// }
...
/* 
Invoke the 'pricesPlusTax' function, passing in the 'prices' array and a callback function (passing in 'totalCost' as a param) that will print 
"The new array plus tax = [totalCost]"
*/

// CODE HERE
// console.log(`The new array plus tax = ` pricesPlusTax(prices, totalCost))
...
The inner function should run this logic: if the first number passing in is greater than and equal to 5, print the 
multiplication of the first and second numbers. If not, print "Cannot multiply: the first number is smaller than 5." 
*/
...
//we know timesFour is a fxn because it is set equal to a fxn returning a fxn

/* 
Now, timesFour is the new function (the inner function that was being returned when we invoked 'multiplyingFactory' function). The number 3 
that we passed in as a first number is now saved in the 'timesFour' function. 
...
/* 
Change the param for 'multiplyingFactory' invocation to number 5. Then invoke 'timesFour' again, passing in number 4. Run the code in node, 
and you should see 20.
*/
timesFour(4)